...
impl AtaDisk {
    ...
    fn wait_not_busy(&mut self) -> Result<(), BlockError> {
        let mut status_port = Port::new(self.io_base + ATA_STATUS);
...
impl BlockDevice for AtaDisk {
    fn read_sector(&mut self, sector: u64, buffer: &mut [u8; SECTOR_SIZE]) -> Result<(), BlockError> {
        ...
        let mut drive_port = Port::new(self.io_base + ATA_DRIVE_SELECT);
        // SAFETY: Writing to ATA drive select port
        unsafe { drive_port.write(drive) };

        // Set sector count to 1
        let mut count_port = Port::new(self.io_base + ATA_SECTOR_COUNT);
        // SAFETY: Writing to ATA sector count port
        unsafe { count_port.write(1u8) };

        // Set LBA (bits 0-7, 8-15, 16-23)
        let mut lba_low = Port::new(self.io_base + ATA_LBA_LOW);
        let mut lba_mid = Port::new(self.io_base + ATA_LBA_MID);
        let mut lba_high = Port::new(self.io_base + ATA_LBA_HIGH);
        ...
        // Send read command
        let mut cmd_port = Port::new(self.io_base + ATA_COMMAND);
        ...
        let mut data_port = Port::new(self.io_base + ATA_DATA);
        for i in 0..256 {
