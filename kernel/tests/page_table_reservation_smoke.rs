...
fn test_allocated_frames_not_page_tables() {
    ...
    for &frame in &allocated_frames {
        for &pt_frame in &pt_frames {
            assert_ne!(frame, pt_frame, "Frame {} was allocated but is a page table frame!", frame);
        }
    }
...
fn test_page_table_frames_reserved() {
    ...
    serial_println!("[ok] - {} frames allocated, none are page tables", allocated_frames.len());
}
...
fn test_heap_and_page_tables_coexist() {
    ...
    for _ in 0..50 {
        if let Some(frame) = unsafe { panda_kernel::memory::allocate_frame() } {
            ...
            assert!(!pt_frames.contains(&frame), "Allocated a page table frame: {}", frame);
...
fn test_page_table_frame_count_stable() {
    ...
    assert_eq!(initial_count, final_count, "Page table count changed unexpectedly");
