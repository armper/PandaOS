            ...
            Some(region) => {
                ...
                let frame = unsafe {
                    crate::memory::allocate_frame()
                        .expect("Failed to allocate frame for demand paging")
                };
                ...
                // Zero-fill the page (for anonymous regions) or populate from ELF
                let page_virt = crate::memory::phys_to_virt(phys_addr);
                if region.file_backed {
                    // File-backed region - would need to read from ELF data
                    // For now, zero-fill (TODO: implement ELF data copying)
                ...
                // Return normally - page fault is handled
                return;
            }
            None => {
            ...
            Some(pte) => {
                ...
                if pte_flags.contains(crate::paging::PageTableFlags::COPY_ON_WRITE) {
                    ...
                    let new_frame = unsafe {
                        crate::memory::allocate_frame().expect("Failed to allocate frame for COW")
                    };
                    ...
                    let new_flags_bits =
                        new_flags.bits() & !crate::paging::PageTableFlags::COPY_ON_WRITE.bits();
                    ...
                    // Return normally - COW fault is handled
                    return;
                }
